...
def make_raspberry_image(output, userdata_size, ptpath,partition_images):
    try:
        os.unlink(output)
...
parser = OptionParser()
parser.add_option('-p', '--ptpath', help='ptgen path')
...
make_raspberry_image(options.output, userdata_size, options.ptpath,args)
